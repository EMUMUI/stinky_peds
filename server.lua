...
-- Player reaction text event 
RegisterNetEvent("stinky:showReactionText", function(targetId, text)
    TriggerClientEvent("stinky:showReactionText", targetId, text)
end)
...
-- Not an export but a function to get the player dirt value client-sided 
...
CreateThread(function()
    while true do 
        ...
        for _, playerId in ipairs(GetPlayers()) do 
            ...
            if identifier then 
                exports.oxmysql:execute('SELECT  dirt FROM player_dirt WHERE identifier = ?', {identifier}, function(result)
                    if result[1] and result[1].dirt >= 100 then 
                        TriggerClientEvent("flies:clientSpawn", -1, GetPlayerPed(playerId))
                    end 
                end)
            end 
        end 
...
RegisterNetEvent("flies:onPlayerTeleported", function()
    local src = source 
    local identifier = getIdentifier(src)
    exports.oxmysql:execute('SELECT  dirt FROM player_dirt WHERE identifier = ?', {identifier}, function(result)
        if result[1] and result[1].dirt >= 100 then 
            TriggerClientEvent("flies:clientSpawn", -1, GetPlayerPed(src))
        end 
    end)
end)
